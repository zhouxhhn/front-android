...
const DoubleScreen = NativeModules.DoubleScreen;

export default class SunmiScreen {
  static models: any;
  static async init(models: any = null) {
    try {
  ...
  static async sendTxtData(data: any) {
    try {
  ...
  static async sendTxtMediaData(data: any) {
    try {
      ...
      let finalData = data.rotation_time
      ...
      if (!data.taskId) {
        data.taskId = Date.parse(new Date().toString()) + '';
      }
      if (typeof _data.media === 'string') {
        const result = await DoubleScreen.sendTxtMediaData(
        ...
        const result = await DoubleScreen.sendImgs(
          JSON.stringify(finalData),
          _data.media,
          data.taskId
        );
  ...
  static async playVideo(data: any) {
    try {
      if (!data.media) {
        return this.error('-1', 'invalid data format');
      }
  ...
  static async playCustom(data: any, media: any, taskId: any) {
    try {
      ...
      switch (data.dataModel) {
        ...
        case this.models.HOT_PATCH:
          result = await DoubleScreen.sendTxtMediaData(
            JSON.stringify(data),
            media,
            taskId
          );
        ...
        case this.models.SHOW_IMGS:
          result = await DoubleScreen.sendImgs(
            JSON.stringify(data),
            media,
            taskId
          );
  ...
  static error(code: any, message: any) {
    let error = {
      code: code,
      message: message
    };
